...
export default function ComponentInfo({ block }: ComponentInfoProps) {
    ...
    const hasJsonSpec = metadata.jsonSpec;
                    ...
                    {hasJsonSpec && (
                        <div className="mt-2 p-2 bg-purple-50 border border-purple-200 rounded text-xs">
                            <div className="flex items-center gap-1 text-purple-700 font-medium mb-1">
                                ...
                                <span>JSON Spec Available</span>
                            </div>
                            <div className="text-purple-600 space-y-0.5">
                                {metadata.jsonSpec.layout && (
                                    <div>Layout: {metadata.jsonSpec.layout.type}</div>
                                )}
                                {metadata.jsonSpec.structure && (
                                    <div>Items: {metadata.jsonSpec.structure.itemCount}</div>
                                )}
                            </div>
                        </div>
                    )}
